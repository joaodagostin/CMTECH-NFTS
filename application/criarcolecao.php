<?php	
	...
	if(isset($_POST['submit'])) {
		...
		$pessoa2 = $pessoa['id'];
		if((!empty($nomecolecao))) {
				
				$foto_colecao = $_FILES['fotocolecao'];
				$pasta_dir = "fotoColecao/";
				if(!file_exists($pasta_dir)) {
					mkdir($pasta_dir);
				}
				$fotonome = $pasta_dir.$foto_colecao['name'];
				move_uploaded_file($foto_colecao['tmp_name'],$fotonome);
		
				$sql2 = mysql_query("INSERT into colecoes(nome_colecao,id_usuario,foto_colecao) values ('$nomecolecao','$pessoa2','$fotonome')") or die (mysql_error());
...
<body>
	...
	<div class="conteudo">
    	<h1>CRIE UMA COLEÇÃO PRÓPRIA</h1>
	</div>  
    <form method="POST" action="criarcolecao.php" enctype="multipart/form-data"> 
                        <div class="fotonft">
							<label class="picture3" for="picture__input3" tabIndex="0">
                				<span class="picture__image3"></span>
            				</label>

            				<input type="file" name="fotocolecao" id="picture__input3">
            				<script type="text/javascript">
						...
						<div class="register-top-grid2">
							<div>
								<span>Nome da coleção<label></label></span>
								<input type="text" name="nomecolecao"> 
							</div>
                		</div>
                <div class="clear"> </div>
                ...
                <div class="clear"> </div>
                <div class="enviar2">
                    <input name="submit" class="submittop2" type="submit" value="Enviar">
                </div>
        </form>
            </div>
			<script>
